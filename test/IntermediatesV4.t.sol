...
contract IntermediatesV4Test is BaseForkFixture {
    ...
    /// @notice 3-hop twin. routeExactOutput3Hop is a separate entrypoint with its own exit and
    /// needs its own guard: it seeds bestQuote from routeExactOutputSingle, which installs
    /// v4QuoteExactOut's uint256.max sentinel directly when the only direct pool cannot fill.
    /// The leg-level sentinel `continue`s inside the 3-hop loops only stop a sentinel from
    /// being folded INTO a combined quote; they do nothing about the seeded candidate.
    /// @dev Note the direct tokenC <-> WETH pool below: it must EXIST but be unfillable. With
    /// no direct pool at all, routeExactOutputSingle returns the empty quote instead of the
    /// sentinel and this entrypoint never leaks, which is why the 2-hop scenario above does
    /// not cover the 3-hop exit.
    function test_routeExactOutput3Hop_unfillableDirectPool_returnsEmptyNotSentinel() public {
        MockERC20 tokenC = new MockERC20("TokenC", "TKC", 18);
        // Direct pool exists but is far too shallow to fill: seeds the sentinel
        _seed(address(tokenC), WETH, int256(1e15));
        // Deep tokenC <-> mid, shallow mid <-> WETH: no multi-hop candidate fills either
        _seed(address(tokenC), address(mid), int256(1e24));
        _seed(address(mid), WETH, int256(1e15));
        router.addIntermediateToken(address(mid));
        ...
        assertEq(q.amountIn, 0, "No-route 3-hop exact-out must quote zero input, never the sentinel");
        assertEq(q.amountOut, 0, "No-route 3-hop exact-out must quote zero output");
        assertEq(q.path.length, 0, "No-route 3-hop exact-out must have an empty path");
    }
